...
namespace appnext
{
    ...
    public class FullscreenVideo : MonoBehaviour
    {
        ...
        public FullscreenVideo(string placementID)
        {
            if (Application.platform != RuntimePlatform.Android)
                return;

            using (var pluginClass = new AndroidJavaClass("com.appnext.unityplugin.AdBuilder"))
                instance = pluginClass.CallStatic<AndroidJavaObject>("getFullScreenVideo", placementID);

            init();
        }

        public FullscreenVideo(string placementID, FullscreenConfig config)
        {
            if (Application.platform != RuntimePlatform.Android)
                return;
        ...
        private void init()
        {
            GameObject interstitialGameObject = GameObject.Find("appnext_fullscreen_gameobject");
            if (interstitialGameObject == null)
            {
                interstitialGameObject = new GameObject("appnext_fullscreen_gameobject");
